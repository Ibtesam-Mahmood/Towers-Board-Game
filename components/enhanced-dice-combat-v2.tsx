...
export function EnhancedDiceCombatV2({ attacker, defender, gameState, onCombatComplete, onCancel }: EnhancedDiceCombatV2Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4">
      <Card className="bg-slate-800/90 backdrop-blur-md border-amber-500/50 w-full max-w-2xl shadow-2xl overflow-hidden">
        <CardHeader className="pb-3 text-center">
          <CardTitle className="text-amber-400 text-2xl flex items-center justify-center gap-2">⚔️ Combat Resolution</CardTitle>
          <p className="text-slate-300 text-sm">{isRanged ? "🏹 Ranged Attack" : "⚔️ Melee Combat"}</p>
        </CardHeader>
        ...
        <CardContent className="space-y-4 overflow-auto max-h-[calc(100vh-8rem)]">
          {phase === "preview" && (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="bg-gradient-to-br from-red-900/30 to-slate-700/30 rounded-lg p-4 border border-red-500/30">
                  ...
                  <div className="text-center space-y-2">
                    <div className="text-2xl mb-2">{attackerTemplate.type === "Infantry" ? "🪖" : attackerTemplate.type === "Cavalry" ? "🐎" : attackerTemplate.type === "Archer" ? "🏹" : attackerTemplate.type === "Artillery" ? "🎯" : "⚔️"}</div>
                ...
                <div className="bg-gradient-to-br from-blue-900/30 to-slate-700/30 rounded-lg p-4 border border-blue-500/30">
                  ...
                  <div className="text-center space-y-2">
                    <div className="text-2xl mb-2">{defenderTemplate.type === "Infantry" ? "🪖" : defenderTemplate.type === "Cavalry" ? "🐎" : defenderTemplate.type === "Archer" ? "🏹" : defenderTemplate.type === "Artillery" ? "🎯" : "⚔️"}</div>
              ...
              <div className="bg-gradient-to-r from-amber-900/20 to-orange-900/20 rounded-lg p-4 text-center border border-amber-500/30">
                <div className="text-lg text-amber-300 mb-2 flex items-center justify-center gap-2">🎲 Combat Mechanics</div>
                <div className="text-sm text-slate-300 space-y-1">
                  <p>Both sides roll 1d6 and add their combat values</p>
                  <div className="grid grid-cols-2 gap-4 mt-3">
                    <div>
                      <p>
                        • <span className="text-yellow-400">Hit (1-2 margin)</span>: 1 damage
                      </p>
                      <p>
                        • <span className="text-red-400">Massive Hit (3+ margin)</span>: 2 damage
                      </p>
                    </div>
                    <div>
                      <p>
                        • <span className="text-blue-400">Tie</span>: Both gain morale
                      </p>
                      <p>
                        • <span className="text-purple-400">Miss</span>: Attacker takes 1 damage
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Button onClick={rollDice} className="bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-bold px-4 sm:px-8 py-3 text-sm sm:text-lg transition-all transform hover:scale-105 shadow-lg min-w-0 flex-1 sm:flex-none whitespace-normal sm:whitespace-nowrap">
                  <span className="break-words text-center">🎲 ROLL FOR COMBAT!</span>
                </Button>
                <Button onClick={onCancel} variant="outline" className="border-slate-500 bg-slate-700/50 px-4 sm:px-8 py-3 hover:bg-slate-600/50 transition-all min-w-0 flex-1 sm:flex-none">
                  Cancel
                </Button>
              </div>
            </>
          ...
          {phase === "rolling" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
                ...
                <div className="text-center">
                  ...
                  <div className="flex justify-center mb-4">
                    <div className={`w-20 h-20 bg-red-500 rounded-lg flex items-center justify-center text-4xl font-bold text-white shadow-lg transition-transform ${diceAnimating ? "animate-spin" : ""}`}>{diceAnimating ? "🎲" : attackerRoll}</div>
                  </div>
                ...
                <div className="text-center">
                  ...
                  <div className="flex justify-center mb-4">
                    <div className={`w-20 h-20 bg-blue-500 rounded-lg flex items-center justify-center text-4xl font-bold text-white shadow-lg transition-transform ${diceAnimating ? "animate-spin" : ""}`}>{diceAnimating ? "🎲" : defenderRoll}</div>
                  </div>
                  <div className="bg-slate-700/50 rounded p-2">
              ...
              <div className="text-center text-amber-300">{diceAnimating ? "Rolling..." : "Dice rolled! Calculating result..."}</div>
            </div>
          )}
          ...
          {phase === "result" && combatResult && (
            <div className="space-y-6">
              <div className="text-center">
                <h3 className="text-3xl font-bold mb-4 animate-bounce">{combatResult.result === "massive-hit" ? "💥 MASSIVE HIT!" : combatResult.result === "hit" ? "🎯 HIT!" : combatResult.result === "miss" ? "❌ MISS!" : "🤝 TIE!"}</h3>
              </div>
              ...
              <div className="bg-gradient-to-r from-amber-900/40 to-orange-900/40 rounded-lg p-4 text-center border border-amber-500/50">
                ...
                {combatResult.damage > 0 ? (
                  <div className="text-lg font-bold mb-2">{combatResult.result === "miss" ? <span className="text-purple-400 flex items-center justify-center gap-2">💔 Attacker takes {combatResult.damage} damage!</span> : <span className="text-red-400 flex items-center justify-center gap-2">💀 Defender takes {combatResult.damage} damage!</span>}</div>
                ) : combatResult.result === "miss" ? (
                  <div className="text-lg font-bold mb-2">
                    <span className="text-slate-400 flex items-center justify-center gap-2">🛡️ Attack missed - no damage dealt!</span>
                  </div>
                ) : null}

                {combatResult.moraleGained > 0 && <div className="text-blue-400 font-bold mb-2">😰 Both units gain {combatResult.moraleGained} morale token!</div>}
              ...
              <div className="text-center">
                <Button onClick={completeCombat} className="bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-bold px-4 sm:px-8 py-3 text-sm sm:text-base transition-all transform hover:scale-105 shadow-lg min-w-0 w-full sm:w-auto">
                  Continue Game
                </Button>
              </div>
            </div>

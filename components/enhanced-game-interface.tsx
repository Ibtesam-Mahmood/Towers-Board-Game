...
export function EnhancedGameInterface({ gameState, onGameStateChange }: EnhancedGameInterfaceProps) {
    ...
    if (actionMode === 'move_attack' && selectedUnit.position) {
      ...
      movePositions.forEach(movePos => {
        ...
        targetsFromPos.forEach(target => {
          if (target.position && !attackPositions.some(pos => 
    ...
    for (let q = 0; q < gameState.boardSize.width; q++) {
      for (let r = 0; r < gameState.boardSize.height; r++) {
        ...
        if ((template.melee > 0 && distance === 1) || 
            (template.ranged > 0 && distance >= 1 && distance <= (template.id === 'siege_engine' ? 3 : 2))) {
          positions.push(pos);
        }
      }
    }
  ...
  const handleHexClick = useCallback((position: HexPosition) => {
    ...
    if (clickedUnit) {
      if (clickedUnit.playerId === gameState.currentPlayer) {
      ...
      } else if (actionMode === 'move_attack' && selectedUnit) {
        // Attack enemy unit (Fire Emblem style)
        const canAttackDirectly = EnhancedGameManager.getValidAttackTargets(selectedUnit, gameState)
          .some(target => target.id === clickedUnit.id);
        ...
        if (canAttackDirectly) {
          ...
          setError('Enemy unit is out of attack range');
        }
      } else {
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-4 items-center">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className={`text-center bg-slate-700/50 rounded-lg p-2 cursor-help ${
                      gameState.activationsRemaining <= 0 ? 'bg-red-900/50' : ''
                    }`}>
            ...
            {showLogs && (
              <div className="mt-3 bg-slate-800/50 rounded p-2 max-h-32 overflow-y-auto">
                <div className="text-xs text-slate-300 space-y-1">
                  {GameLogger.getLogs(10).reverse().map((log, idx) => (
                    <div key={log.id} className="flex items-start gap-2">
                      <span className={`px-1 rounded text-xs font-mono ${
                        log.type === 'ERROR' ? 'bg-red-600' :
                        log.type === 'COMBAT' ? 'bg-orange-600' :
                        log.type === 'MOVEMENT' ? 'bg-green-600' :
                        log.type === 'DEPLOYMENT' ? 'bg-purple-600' :
                        'bg-slate-600'
                      }`}>
                        {log.type}
                      </span>
                      ...
                      <div className="absolute inset-0 flex items-center justify-center bg-black/50 z-50">
                        <div className="bg-slate-800 border border-amber-500 rounded-lg p-6 text-center">
                          <div className="text-2xl font-bold mb-2">
                            {lastCombatResult.result === 'hit' ? '🎯 HIT!' : 
                             lastCombatResult.result === 'massive-hit' ? '💥 MASSIVE HIT!' :
                             lastCombatResult.result === 'miss' ? '❌ MISS!' : '🤝 TIE!'}
                          </div>
                          {lastCombatResult.damage > 0 && (
                            <div className="text-red-400 text-lg font-bold">
                              -{lastCombatResult.damage} HP
                            </div>
                          )}
                        </div>
                      </div>
              ...
              <Tabs defaultValue="units" className="w-full">
                ...
                <TabsContent value="info">
                  <Card className="bg-slate-800/50 border-slate-600">
                    ...
                    <CardContent>
                      {selectedUnit ? (
                        <EnhancedUnitInfo unit={selectedUnit} gameState={gameState} />
                      ) : (
                        ...
                        {gameState.combatLog.slice(-3).reverse().map(combat => (
                          <div key={combat.id} className="bg-slate-700/50 rounded p-2 text-xs">
                            <div className="font-semibold mb-1 text-amber-300">
                              {combat.result === 'massive-hit' ? '💥 Massive Hit!' :
                            ...
                            {combat.specialEffects.length > 0 && (
                              <div className="text-purple-400 text-xs mt-1">
                                ✨ {combat.specialEffects.join(', ')}
                              </div>
                            )}
                          </div>
                        ))}
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <div 
            className={`bg-slate-700/50 rounded p-2 border cursor-pointer transition-all hover:bg-slate-600/50 ${
      ...
      {!isDead && unit.isDeployed && (
        <div className="bg-slate-700/50 rounded p-2 space-y-1">
          ...
          <div className="text-xs text-slate-300 space-y-1">
            ...
            {unit.position && (
              <div>Position: ({unit.position.q},{unit.position.r})</div>
            )}
          </div>
        </div>
      )}
      ...
      <Card className="bg-slate-800 border-slate-600 w-full max-w-lg">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-slate-700/50 rounded-lg p-3">
              ...
              <div className="text-center space-y-2">
                ...
                {attacker.moraleTokens > 0 && (
                  <div className="text-orange-400 text-xs">
                    😰 Morale: {attacker.moraleTokens}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-700/50 rounded-lg p-3">
              ...
              <div className="text-center space-y-2">
                ...
                {defender.moraleTokens > 0 && (
                  <div className="text-orange-400 text-xs">
                    😰 Morale: {defender.moraleTokens}
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="flex gap-3">
            <Button 
              onClick={onConfirm} 
              className="flex-1 bg-red-600 hover:bg-red-700 text-white font-semibold text-lg py-3"
            >
              ⚔️ ATTACK!
            </Button>
            <Button 
              onClick={onCancel} 
              variant="outline" 
              className="flex-1 border-slate-500 py-3"
            >
              Cancel
            </Button>
          </div>

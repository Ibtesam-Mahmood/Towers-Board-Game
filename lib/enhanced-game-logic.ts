...
import { deployUnit, moveUnit, endTurn, isInDeploymentZone, cleanupDeadUnits } from './game-state-manager';
...
export type LogType = 'ACTION' | 'COMBAT' | 'DEPLOYMENT' | 'MOVEMENT' | 'STATE_CHANGE' | 'ERROR' | 'VALIDATION' | 'AI';
...
export class EnhancedGameManager {
  ...
  static removeDeadUnits(gameState: GameState): GameState {
    ...
    Object.keys(newState.units).forEach(unitId => {
      ...
      if (unit.currentHp <= 0 && unit.isDeployed) {
        ...
        if (newState.players[unit.playerId]) {
          newState.players[unit.playerId].deployedUnits = Math.max(0, 
    ...
    if (unitsRemoved > 0) {
      GameLogger.log('STATE_CHANGE', `Removed ${unitsRemoved} dead units from battlefield`);
    }
  ...
  static deployUnitEnhanced(unitId: string, position: HexPosition, gameState: GameState): GameState {
    GameLogger.log('DEPLOYMENT', `Attempting to deploy unit ${unitId}`, { position });
    ...
    try {
      ...
      GameLogger.log('ERROR', 'Deployment failed', { unitId, position, error: error instanceof Error ? error.message : 'Unknown error' });
      throw error;
    }
  }
  ...
  static undeployUnit(unitId: string, gameState: GameState): GameState {
    ...
    if (unit.playerId !== gameState.currentPlayer) {
      ...
      GameLogger.log('ERROR', error, { unitId, currentPlayer: gameState.currentPlayer });
      throw new Error(error);
    }
  ...
  static moveUnitEnhanced(unitId: string, targetPosition: HexPosition, gameState: GameState): GameState {
    GameLogger.log('MOVEMENT', `Attempting to move unit ${unitId}`, { targetPosition });
    ...
    if (gameState.activationsRemaining <= 0) {
      ...
      GameLogger.log('ERROR', error, { activationsRemaining: gameState.activationsRemaining });
      throw new Error(error);
    }
    ...
    try {
      ...
      newState.activationsRemaining = Math.max(0, newState.activationsRemaining);
      ...
      GameLogger.log('ERROR', 'Movement failed', { unitId, targetPosition, error: error instanceof Error ? error.message : 'Unknown error' });
      throw error;
    }
  }

  // Enhanced combat with proper validation and death handling
  static executeCombat(attackerId: string, defenderId: string, gameState: GameState): GameState {
    ...
    try {
      ...
      const combatResult = resolveCombat(attackerId, defenderId, gameState, isRanged);
      ...
      if (combatResult.damage > 0) {
        if (combatResult.result === 'miss') {
          ...
          GameLogger.log('COMBAT', `Attacker took ${combatResult.damage} damage`, { 
            newHp, 
          ...
          GameLogger.log('COMBAT', `Defender took ${combatResult.damage} damage`, { 
            newHp, 
      ...
      if (combatResult.moraleGained > 0 && combatResult.result === 'tie') {
        newState.units[attackerId] = {
          ...newState.units[attackerId],
          moraleTokens: newState.units[attackerId].moraleTokens + combatResult.moraleGained
        };
        newState.units[defenderId] = {
          ...newState.units[defenderId],
          moraleTokens: newState.units[defenderId].moraleTokens + combatResult.moraleGained
        };
        GameLogger.log('COMBAT', `Both units gained ${combatResult.moraleGained} morale tokens`);
      }
  ...
  static getValidMovementPositions(unit: Unit, gameState: GameState): HexPosition[] {
  ...
  static getValidAttackTargets(unit: Unit, gameState: GameState): Unit[] {
    ...
    Object.values(gameState.units).forEach(target => {
      if (target.isDeployed && target.currentHp > 0 && target.playerId !== unit.playerId && target.position) {
  ...
  static endTurnEnhanced(gameState: GameState): GameState {
    ...
    try {
      ...
      GameLogger.log('ERROR', 'Failed to end turn', { error: error instanceof Error ? error.message : 'Unknown error' });
      throw error;
    }
  }

  // Victory condition checking with logging
  static checkVictoryConditions(gameState: GameState): { winner: string | null; reason: string } {
    // Check during both deployment and battle phases
    const player1AliveUnits = Object.values(gameState.units).filter(
      unit => unit.playerId === 'player1' && (unit.isDeployed || unit.isInReserves) && unit.currentHp > 0
    );
    
    const player2AliveUnits = Object.values(gameState.units).filter(
      unit => unit.playerId === 'player2' && (unit.isDeployed || unit.isInReserves) && unit.currentHp > 0
    );
    ...
    if (gameState.currentPhase === 'battle') {
      if (player1AliveUnits.length === 0 && player2AliveUnits.length > 0) {
        GameLogger.log('STATE_CHANGE', 'Player 2 wins skirmish - Player 1 eliminated');
        return { winner: 'player2', reason: 'Player 1 has no remaining units' };
      }

      if (player2AliveUnits.length === 0 && player1AliveUnits.length > 0) {
        GameLogger.log('STATE_CHANGE', 'Player 1 wins skirmish - Player 2 eliminated');
  ...
  static autoDeployUnits(playerId: string, gameState: GameState): GameState {
    ...
    const unitsToAutoDeploy = Object.values(newState.units).filter(
      unit => unit.isInReserves && unit.playerId === playerId && unit.currentHp > 0
    );
    ...
    for (let q = 0; q < gameState.boardSize.width; q++) {
      for (let r = 0; r < gameState.boardSize.height; r++) {
        ...
        if (!occupyingUnit && isInDeploymentZone(position, playerId, newState)) {
    ...
    GameLogger.log('DEPLOYMENT', `Auto-deployed ${deployedCount} units for ${playerId}`);
    return newState;
  }

  // Basic AI auto-move functionality
  static suggestMove(playerId: string, gameState: GameState): { unitId: string; action: 'move' | 'attack'; target: HexPosition | string } | null {
    if (gameState.currentPhase !== 'battle' || gameState.currentPlayer !== playerId) {
      return null;
    }

    // Get all available units that can act
    const availableUnits = Object.values(gameState.units).filter(
      unit => unit.playerId === playerId && unit.isDeployed && !unit.activated && unit.currentHp > 0
    );
    ...
    for (const unit of availableUnits) {
      ...
      if (movePositions.length > 0) {
        ...
        const enemyUnits = Object.values(gameState.units).filter(
          u => u.playerId !== playerId && u.isDeployed && u.currentHp > 0 && u.position
        );
        ...
        if (enemyUnits.length > 0 && unit.position) {
          ...
          for (const position of movePositions) {
            const avgDistanceToEnemies = enemyUnits.reduce((sum, enemy) => 
              sum + hexDistance(position, enemy.position!), 0) / enemyUnits.length;
  ...
  static executeAIMove(playerId: string, gameState: GameState): GameState {
    ...
    try {
      ...
      if (suggestion.action === 'move') {
        newState = this.moveUnitEnhanced(suggestion.unitId, suggestion.target as HexPosition, gameState);
        GameLogger.log('AI', `AI moved unit ${suggestion.unitId}`, { target: suggestion.target });
      } else if (suggestion.action === 'attack') {
        newState = this.executeCombat(suggestion.unitId, suggestion.target as string, gameState);
        GameLogger.log('AI', `AI attacked with unit ${suggestion.unitId}`, { target: suggestion.target });
      }

      return newState;
    } catch (error) {
      GameLogger.log('ERROR', 'AI move execution failed', { suggestion, error });
      return gameState;
    }
  }
}

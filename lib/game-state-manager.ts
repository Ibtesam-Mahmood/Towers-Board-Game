...
export function generateInitialTerrain(width: number, height: number): { [key: string]: string } {
  ...
  terrain[positionToKey({ q: Math.floor(width * 0.25), r: player1SupplyCampRow })] = 'supply_camp';
  terrain[positionToKey({ q: Math.floor(width * 0.75), r: player1SupplyCampRow })] = 'supply_camp';
  terrain[positionToKey({ q: Math.floor(width * 0.25), r: player2SupplyCampRow })] = 'supply_camp';
  terrain[positionToKey({ q: Math.floor(width * 0.75), r: player2SupplyCampRow })] = 'supply_camp';
  
  return terrain;
}

export function buildArmy(playerId: string, armyList: string[], gameState: GameState): GameState {
  ...
  if (totalCost > DEFAULT_CONFIG.pointLimit) {
    throw new Error(`Army exceeds point limit: ${totalCost}/${DEFAULT_CONFIG.pointLimit}`);
  }
...
export function deployUnit(unitId: string, position: HexPosition, gameState: GameState): GameState {
  ...
  // Count only alive deployed units for deployment limit
  ...
  if (aliveDeployedUnits >= player.maxDeployment) {
    throw new Error(`Maximum deployment reached (${player.maxDeployment} units)`);
  }
  ...
  newState.players[unit.playerId].deployedUnits = aliveDeployedUnits + 1;
  
  return calculateSupplyStatus(newState);
}

export function isInDeploymentZone(position: HexPosition, playerId: string, gameState: GameState): boolean {
...
export function startTurn(gameState: GameState): GameState {
  ...
  // Add CP (up to max)
  currentPlayer.cp = Math.min(currentPlayer.cp + DEFAULT_CONFIG.cpPerTurn, DEFAULT_CONFIG.maxCp);
...
export function moveUnit(unitId: string, targetPosition: HexPosition, gameState: GameState): GameState {
  const unit = gameState.units[unitId];
  ...
  if (unit.playerId !== gameState.currentPlayer) throw new Error('Not your unit');
  if (gameState.activationsRemaining <= 0) throw new Error('No activations remaining');
  ...
  if (distance > template.move) throw new Error('Target position out of movement range');
...
export function checkVictoryConditions(gameState: GameState): { winner: string | null; reason: string } {
  ...
  if (player1AliveUnits.length === 0 && player2AliveUnits.length > 0) {
    return { winner: 'player2', reason: 'Player 1 has no remaining alive units' };
  }
  
  if (player2AliveUnits.length === 0 && player1AliveUnits.length > 0) {
    return { winner: 'player1', reason: 'Player 2 has no remaining alive units' };
  }
  
  // Check match victory (best of 3)
